...
export default function PhoneVerification() {
  ...
  const { signIn, getTempCredentials } = useAuth();
  ...
  const handleVerification = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const { success } = await authService.verifyPhone({
        token,
        code: verificationCode,
      });
      
      if (success) {
        const credentials = getTempCredentials();
        if (!credentials) {
          toast.error('Credenciais não encontradas. Por favor, faça login manualmente.');
          navigate('/login');
          return;
        }
  ...
  const handleResendCode = async () => {
    try {
      setResending(true);
      await authService.resendCode({ token, type: 'phone' });
